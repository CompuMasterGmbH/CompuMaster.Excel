...
Namespace ExcelOps
    ...
    Partial Public Class EpplusFreeExcelDataOperations
        ...
        Public Overrides Function LookupCellValue(Of T)(cell As ExcelCell) As T
            ...
            If Me.IsMissingCalculatedCellValueFromFormulaCell(cell) Then Throw New MissingCalculatedCellValueException(Me.FilePath, cell, Me.LookupCellFormula(cell))
            Try
        ...
        '''' <returns></returns>
        '''' <remarks>Cell values with spaces will be converted to null values in case of method call with types bool, byte, int32, int64, double, decimal</remarks>
        'Private Overloads Function LookupCellValue(Of T)(cell As EpplusFreeOfficeOpenXml.ExcelRange) As T
        '    Select Case GetType(T)
        '        Case GetType(Integer), GetType(Long), GetType(Byte), GetType(Double), GetType(Decimal), GetType(Boolean)
...
#End Region
        ...
        Public Overrides Function LookupLastCell(sheetName As String) As ExcelOps.ExcelCell
            ...
            Dim Result As ExcelCell
            If Sheet.Dimension Is Nothing Then
                Result = New ExcelOps.ExcelCell(sheetName, "A1", Nothing)
            Else
                Result = New ExcelOps.ExcelCell(sheetName, Sheet.Dimension.End.Address, Nothing)
            End If
            Dim LastMergedCell = Me.FindLastMergedCell(sheetName)
            If LastMergedCell IsNot Nothing Then
...
#End Region
        ...
        '''' <returns></returns>
        '''' <remarks></remarks>
        'Private Overloads Function LookupCellFormattedText(ByVal cell As EpplusFreeOfficeOpenXml.ExcelRange) As String
        '    Try
        '        Dim CellValue As Object = cell.Value
        ...
        Public Overrides Sub CopySheetContentInternal(sheetName As String, targetWorkbook As ExcelDataOperationsBase, targetSheetName As String)
            If sheetName = Nothing Then Throw New ArgumentNullException(NameOf(sheetName))
            If targetWorkbook.GetType IsNot GetType(EpplusFreeExcelDataOperations) Then Throw New NotSupportedException("Excel engines must be the same for source and target workbook for copying worksheets")
            ...
            Me.Workbook.Worksheets(sheetName).Cells(1, 1, LastCell.RowIndex + 1, LastCell.ColumnIndex + 1).Copy(CType(targetWorkbook, EpplusFreeExcelDataOperations).Workbook.Worksheets(targetSheetName).Cells)
